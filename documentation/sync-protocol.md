...
Two installations reporting the same `device_id` (for example, a device restored from another device's backup without regenerating its identifier) is a possible failure mode.

> [!note] Open question
> The brief flags "duplicate device IDs" as an edge case the protocol must survive, but does not specify the detection or resolution mechanism — for instance, whether `device_id` collisions are detected server-side and force a client-side identifier regeneration, whether they are tolerated because all conflict resolution is keyed on `event_id`/`id`/`updated_at` rather than `device_id`, or whether `device_id` participates in any uniqueness constraint at all. This needs to be resolved against [[database-schema]] and [[security]] (device/session identity) before implementation.

### Overlap Rules

`activity_events` from the same device can legitimately overlap in wall-clock time only as a data-quality artifact (for example, a tracking bug or a monitor restart producing a slightly re-drawn interval); the server trims overlapping intervals coming from the **same** device so that a single device never reports more active time in a window than the window contains.
