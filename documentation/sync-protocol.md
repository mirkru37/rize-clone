...
Two installations reporting the same `device_id` (for example, a device restored from another device's backup without regenerating its identifier) is a possible failure mode. The protocol tolerates it by construction rather than detecting or resolving it server-side:

- **Uniqueness is scoped per user, not global.** The constraint semantics are `UNIQUE (user_id, device_id)` (see [[database-schema]]'s `devices` table). Collisions **across different users** are tolerated and harmless: every row in every syncable table is tenant-scoped by `user_id`, so two users' data can never mix regardless of what `device_id` each of their installs happens to report. Device ids are client-generated UUIDs, which makes a cross-user collision practically impossible on top of being harmless even if it occurred.
...
- **Prevention is client-side, not server-side.** Clients store their `device_id` in Keychain with `ThisDeviceOnly` accessibility, so the identifier is excluded from device backups and never migrates to a restored installation. This is the actual mitigation for the backup-restore case; there is no server-side collision detection or forced regeneration mechanism. Enforcing this consistently on the mobile client is tracked as a follow-up in the epic's cleanup issue.
...
Instead, trimming happens at report/aggregation time: [[architecture-backend]]'s aggregation logic caps, per device, the active time a single device can contribute to a given window, so that a single device never contributes more active time to a window than the window contains — regardless of how its raw, overlapping events were originally reported. This is the same stage at which cross-device overlap is already disambiguated (see below), so both same-device trimming and cross-device disambiguation are handled by one piece of logic at one stage of the pipeline.

Overlapping events from **different** devices are allowed to stand as reported and are not trimmed against each other — a user genuinely can have simultaneous desktop and mobile activity — and are disambiguated at report time (by [[architecture-backend]]'s aggregation logic) rather than at ingestion time.
